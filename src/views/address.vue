<template>
    <div>
              ...
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkedIndex==index}" @click="checkedIndex = index">
                    ...
                    <div class="addr-opration addr-default" v-if="item.isDefault" ><i>Default address</i></div>
                  </li>
                  <li class="addr-new" @click="addConfirm=true; errorTip=false;">
      ...
      <!-- 删除地址确认框 -->
      <model v-bind:mdShow="mdConfirm" v-on:close="closeModal">
        <p slot="message">
          确认删除此条地址信息？
        </p>
        <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:(0);" @click="delAddress" >确认</a>
          <a class="btn btn--m" href="javascript:(0);" @click="mdConfirm = false">关闭</a>
        </div>
      </model>
      <!-- 新增地址框 -->
      <model v-bind:mdShow="addConfirm" v-on:close="closeModal">
        <div slot="message">
          <p style="padding-bottom: 25px;">添加新地址</p>
          ...
          <div class="confirm-tips">
            ...
            <ul>
              <li class="regi_form_input">
                <input style="left: 15px;" type="text" tabindex="1" name="userName" placeholder="User Name" v-model="userName" data-type="loginname" class="regi_login_input regi_login_input_left">
              </li>
              <li class="regi_form_input noMargin">
                <input style="left: 15px;" type="text" tabindex="2" name="streetName" placeholder="street Name" v-model="streetName"   class="regi_login_input regi_login_input_left ">
              </li>
              <li class="regi_form_input noMargin">
                <input style="left: 15px;" type="text" tabindex="3" name="postCode" placeholder="Post Code" v-model="postCode"   class="regi_login_input regi_login_input_left login-input-no input_text">
              </li>
              <li class="regi_form_input noMargin">
                <input style="left: 15px;" type="text" tabindex="4" name="tel" placeholder="Tel" v-model="tel"   class="regi_login_input regi_login_input_left login-input-no input_text">
              </li>
            </ul>
          </div>
          <div class="login-wrap"><a href="javascript:;" class="btn-login" @click="addAddress">添  加</a></div>
        </div>
        </div>
      </model>
...
<script>
  ...
  import axios from 'axios'
    export default {
      ...
      methods:{
          init(){
            axios.get('/users/adressList').then((res)=>{
